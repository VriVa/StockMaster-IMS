    ...
    <div className="overflow-x-auto">
      <table className="w-full">
        <thead className="bg-[#FBF8F4] border-b border-[#D7CCC8]">
          <tr>
            <th className="px-6 py-4 text-left text-xs font-semibold text-[#5D4037] uppercase tracking-wider">Reference</th>
            <th className="px-6 py-4 text-left text-xs font-semibold text-[#5D4037] uppercase tracking-wider">From Warehouse</th>
            <th className="px-6 py-4 text-left text-xs font-semibold text-[#5D4037] uppercase tracking-wider">To Warehouse</th>
            <th className="px-6 py-4 text-left text-xs font-semibold text-[#5D4037] uppercase tracking-wider">Contact</th>
            <th className="px-6 py-4 text-left text-xs font-semibold text-[#5D4037] uppercase tracking-wider">Scheduled Date</th>
            <th className="px-6 py-4 text-left text-xs font-semibold text-[#5D4037] uppercase tracking-wider">Status</th>
          </tr>
        </thead>
...
const ReceiptsListPage = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [receiptsData, warehousesData] = await Promise.all([
        getTransactionsByType('receipt'),
        getWarehouses()
      ]);
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div className="mb-6">
          <div className="relative max-w-md">
            ...
            <input
              type="text"
              placeholder="Search by reference number or contact..."
              value={searchQuery}
              onChange={(e) => {
                  ...
                  <tbody className="divide-y divide-[#D7CCC8]">
                    {paginatedReceipts.map((receipt) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-2">
                            <WarehouseIcon size={16} className="text-[#8D6E63]" />
              ...
              {paginatedReceipts.map((receipt) => (
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2 text-sm">
                      <ArrowRight size={14} className="text-[#8D6E63]" />
                    ...
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className="text-[#5D4037]">{receipt.contact || '-'}</span>
                    </div>
                    
                    <div className="flex items-center gap-2 text-sm">
                      <Calendar size={14} className="text-[#8D6E63]" />
                      <span className="text-[#5D4037]">{formatDate(receipt.scheduled_date)}</span>
                    </div>
                  </div>
                </div>
              ))}
