...
export const getWarehouse = (id) => Promise.resolve(warehouses.find(w => w.id === id));
...
export const getTransactionsByWarehouse = (warehouseId) =>
  Promise.resolve(transactions.filter(t => 
    t.from_warehouse === parseInt(warehouseId) || t.to_warehouse === parseInt(warehouseId)
  ));

// TRANSACTION LINES
export const getTransactionLines = (txnId) =>
  Promise.resolve(transactionLines.filter(tl => tl.transaction_id === parseInt(txnId)));
...
export const getStockLedgerByProduct = (productId) =>
  Promise.resolve(stockLedger.filter(entry => entry.product_id === parseInt(productId)));
export const getStockLedgerByWarehouse = (warehouseId) =>
  Promise.resolve(stockLedger.filter(entry => entry.warehouse_id === parseInt(warehouseId)));

// USERS
export const getUsers = () => Promise.resolve(users);
export const getUser = (id) => Promise.resolve(users.find(u => u.id === parseInt(id)));
...
export const updateStock = (warehouseId, productId, updates) => {
  const index = stock.findIndex(s => 
    s.warehouse_id === parseInt(warehouseId) && s.product_id === parseInt(productId)
  );
  if (index !== -1) {
...
export default {
  ...
  getStockByWarehouse: (warehouseId) => withDelay(getStockByWarehouse(warehouseId)),
  getStockByProduct: (productId) => withDelay(getStockByProduct(productId)),
  updateStock: (warehouseId, productId, updates) => withDelay(updateStock(warehouseId, productId, updates)),
  ...
  getTransactionsByWarehouse: (warehouseId) => withDelay(getTransactionsByWarehouse(warehouseId)),
  ...
  getStockLedgerByProduct: (productId) => withDelay(getStockLedgerByProduct(productId)),
  getStockLedgerByWarehouse: (warehouseId) => withDelay(getStockLedgerByWarehouse(warehouseId)),
